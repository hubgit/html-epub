...
class HTMLEPUB {
  ...
  stream(outputStream) {
    ...
    return new Promise((resolve, reject) => {
      ...
      archive.append(builder.create(this.container, { encoding: 'UTF-8' }).end({
        pretty: true
      }), { name: 'META-INF/container.xml' });

      // EPUB/package.opf - describes the package contents
      archive.append(builder.create(this.package, { encoding: 'UTF-8' }).end({
        pretty: true
      }), { name: 'EPUB/package.opf' });

      // EPUB/xhtml/toc.xhtml - table of contents
      archive.append(builder.create(this.toc, { encoding: 'UTF-8' }).dtd().end({
        pretty: true
      }), { name: 'EPUB/toc.xhtml' });
  ...
  extract($) {
    ...
    // TODO: cheerio adds a forward slash at the start of relative URLs
    const resourceRoot = epub.options.resourceRoot || '';
