...
class HTMLEPUB {
    ...
    return new Promise((resolve, reject) => {
      doc.tidyBuffer(html, (err, result) => {
        if (err) {
          ...
          $('meta[name="generator"]').attr('content', 'HTMLEPUB')
          resolve($)
        }
      })
    })
  ...
  loadHTMLFiles (htmlFiles) {
    return Promise.all(htmlFiles.map((htmlFile) => {
      return this.parseHTML(htmlFile.content).then($ => {
        const id = uuid()
  ...
  get manifest () {
    ...
    this.xhtml.forEach(item => {
      ...
      const properties = this.properties(item.$)
  ...
  properties ($) {
    ...
    // 'scripted' if contains scripted content and/or HTML form elements
    if ($('script, form, :input').length) {
      properties.push('scripted')
    }

    // 'mathml' if contains MathML markup
    if ($('math, mml\\:math').length) {
      properties.push('mathml')
    }

    // 'svg' if contains SVG markup
    if ($('svg, svg\\:svg').length) {
      properties.push('svg')
    }
